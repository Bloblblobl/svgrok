...
import Old.Path exposing (Index2)
...
type alias AnnotatedPath =
    { commands : List Old.Path.Command
    , selected : Set Index2
...
type alias Model =
    ...
    , grid : Maybe Old.Path.Grid
    }


initAnnotatedPath : List Old.Path.Command -> AnnotatedPath
        ...
        PathStringChanged newPathString ->
            let
                ( commands, errorString ) =
                    Old.Path.fromString newPathString
            in
            { model
        ...
        MouseMoveOverlay offset ->
            let
                ...
                transformedOffset =
                    ...
                    case model.grid of
                        Just grid ->
                            Old.Path.snapToGrid grid viewBoxPoint

                        Nothing ->
                            viewBoxPoint

                updatedCommands : List Old.Path.Command
                updatedCommands =
                    case model.dragging of
                        Just index ->
                            Old.Path.updateCommands
                                index
                                transformedOffset
                                model.path.commands
                ...
                updatedPathString =
                    Old.Path.commandsToString updatedCommands
            in
            { model
                | mouseOffset = offset
...
elementToViewBoxPoint config point =
    ...
    let
        ...
        viewBox : Old.Path.Rect
        viewBox =
            Old.Path.viewBoxRectFromString config.viewBox
...
viewLine overlayBuilder endPoint =
    let
        ...
        pathString : String
        pathString =
            Old.Path.absoluteCommandOverlayString
                overlayBuilder.currentPoint
                (Old.Path.AbsoluteLine endPoint)
...
viewCubicCurve overlayBuilder controls endPoint =
    let
        ...
        pathString =
            Old.Path.absoluteCommandOverlayString
                overlayBuilder.currentPoint
                (Old.Path.AbsoluteCubicCurve controls endPoint)
...
viewQuadraticCurve overlayBuilder control endPoint =
    let
        ...
        pathString =
            Old.Path.absoluteCommandOverlayString
                overlayBuilder.currentPoint
                (Old.Path.AbsoluteQuadraticCurve control endPoint)
...
viewArc : OverlayBuilder -> Old.Path.ArcParameters -> Point -> OverlayBuilder
viewArc overlayBuilder params endPoint =
    ...
    let
        ...
        pathString =
            Old.Path.absoluteCommandOverlayString
                overlayBuilder.currentPoint
                (Old.Path.AbsoluteArc params endPoint)
...
viewOverlayStep : Old.Path.AbsoluteCommand -> OverlayBuilder -> OverlayBuilder
viewOverlayStep absoluteCommand overlayBuilder =
    case absoluteCommand of
        Old.Path.AbsoluteMove endPoint ->
            viewMove overlayBuilder endPoint
        ...
        Old.Path.AbsoluteArc parameters endPoint ->
            viewArc overlayBuilder parameters endPoint


viewOverlayGrid : Config -> Old.Path.Grid -> Html Msg
...
viewOverlay { config, path, grid } =
    let
        absoluteCommands : List Old.Path.AbsoluteCommand
        absoluteCommands =
            Old.Path.resolveRawCommands path.commands
...
viewCommandStrings : List Old.Path.Command -> Html Msg
viewCommandStrings commands =
    let
        commandListItems : List (Html Msg)
        commandListItems =
            List.map Old.Path.commandToString commands
                |> List.map Html.text
